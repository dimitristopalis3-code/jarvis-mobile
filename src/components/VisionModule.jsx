...
const VisionModule = () => {
  ...
  // 2. START CAMERA (Re-runs when facingMode changes)
  useEffect(() => {
    let stream = null;
  ...
  const drawReactorLock = (ctx, x, y, w, h, color, time) => {
    ...
    if(facingMode === 'user') ctx.scale(-1, 1); // Mirror effect for selfie cam
      ...
      if (videoRef.current && videoRef.current.readyState === 4 && canvasRef.current) {
        ...
        resizedDetections.forEach((detection, index) => {
            ...
            if (isPrimary) {
                ...
                // INFO (Correct text direction even if mirrored)
                ctx.save();
                if (facingMode === 'user') { ctx.scale(-1, 1); ctx.translate(-(box.x * 2 + box.width + 180), 0); } // Fix text flip
